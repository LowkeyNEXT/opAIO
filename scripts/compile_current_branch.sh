...
prepare_dockerfile() {
  ...
  if ! grep -q '/usr/local/lib/libLLVM.so' "$patched_dockerfile"; then
    perl -0pi -e 's/\nENV NVIDIA_VISIBLE_DEVICES/\nRUN lib="\$(find \/lib \/usr\/lib -name '\''libLLVM-*.so.1'\'' -print 2>\/dev\/null | sort -V | tail -n 1)" \&\& if [ -n "\$lib" ]; then ln -sf "\$lib" \/usr\/local\/lib\/libLLVM.so \&\& ldconfig; fi\n\nENV NVIDIA_VISIBLE_DEVICES/' "$patched_dockerfile"
  fi
